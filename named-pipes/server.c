...
int main() {
	...
	HANDLE hFile = CreateFile(fileName, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 
		FILE_ATTRIBUTE_NORMAL, NULL); // Всегда создаём новый файл (даже если его не существует). 
	...
	HANDLE hMapping = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, 0, NULL); // Создание отображения файла.
	if (hMapping == NULL) {
		printf("CreateFileMapping failed, %d\n", GetLastError());
		return -1;
	}

	pData = (struct Employee*)MapViewOfFile(hMapping, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, dwFileSize); // Отображение файла в память.
	...
	UnmapViewOfFile(pData); // отмена отображения файла.
